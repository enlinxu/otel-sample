...
docker build -t otel-sample/order-service:latest "${ORDER_DIR}"

echo "Building inventory-service image..."
docker build -t otel-sample/inventory-service:latest "${INVENTORY_DIR}"
...
load_image otel-sample/order-service:latest
load_image otel-sample/inventory-service:latest
