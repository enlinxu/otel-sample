...
docker build -t otel-sample-zero/order-service:latest "${ORDER_DIR}"

echo "Building inventory-service image..."
docker build -t otel-sample-zero/inventory-service:latest "${INVENTORY_DIR}"
...
load_image otel-sample-zero/order-service:latest
load_image otel-sample-zero/inventory-service:latest
